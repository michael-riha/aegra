...
async def wait_for_stateless_run(
    request: RunCreate,
    user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
) -> dict[str, Any]:
    """Create a stateless run, execute it, and wait for completion (Agent Protocol).

    Creates a temporary thread, executes the run, and waits for completion.
    Returns the final output directly (not the Run object).

    Compatible with LangGraph SDK's runs.wait(thread_id=None, ...) pattern.
    """
    # Create temporary thread for stateless run
    thread_id = str(uuid4())

    logger.info(
        f"[wait_for_stateless_run] creating temporary thread thread_id={thread_id} user={user.identity}"
    )

    # Build metadata for temporary thread
    metadata = {
        "owner": user.identity,
        "assistant_id": None,
        "graph_id": None,
    ...
    try:
        # Reuse the stateful wait logic
        run_id = str(uuid4())

        # Get LangGraph service
        langgraph_service = get_langgraph_service()
        logger.info(
            f"[wait_for_stateless_run] creating run run_id={run_id} thread_id={thread_id} user={user.identity}"
        )
        ...
        logger.info(
            f"[wait_for_stateless_run] background task created task_id={id(task)} for run_id={run_id}"
        )
        active_runs[run_id] = task
        ...
        try:
            ...
            logger.warning(
                f"[wait_for_stateless_run] timeout waiting for run_id={run_id}"
            )
            # Don't raise, just return current state
        except asyncio.CancelledError:
            logger.info(f"[wait_for_stateless_run] cancelled run_id={run_id}")
            # Task was cancelled, continue to return final state
        except Exception as e:
            logger.error(f"[wait_for_stateless_run] exception in run_id={run_id}: {e}")
            # Exception already handled by execute_run_async
        ...
        # Note: We don't automatically delete the temporary thread here.
        # Users can control cleanup via on_completion parameter if needed.
        # For now, temporary threads remain in the database for debugging/auditing.
        logger.info(
            f"[wait_for_stateless_run] completed with temporary thread thread_id={thread_id}"
        )
